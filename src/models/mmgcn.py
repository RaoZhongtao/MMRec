...
class MMGCN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.t_feat is not None:
            self.t_gcn = GCN(self.edge_index, batch_size, num_user, num_item, self.t_feat.size(1), dim_x,
                             self.aggr_mode, self.concate, num_layer=num_layer, has_id=has_id, dim_latent=384, device=self.device)
            self.num_modal += 1
    ...
    def fixed_samples_sort_predict(self, interaction):
        ...
        # print(f'\033[91m debugging mmgcn fixed_samples_sort_predict neg_items[0] len {length} \033[0m')
